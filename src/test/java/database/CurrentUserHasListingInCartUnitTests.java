package database;

import Main.Main;
import entities.Listing;
import entities.User;
import org.junit.jupiter.api.AfterAll;
import org.junit.jupiter.api.BeforeAll;
import org.junit.jupiter.api.BeforeEach;
import org.junit.jupiter.api.Test;

import java.io.File;
import java.io.IOException;
import java.time.LocalDate;

public class CurrentUserHasListingInCartUnitTests {
    public static final DatabaseController db = new DatabaseController();

    @BeforeAll
    public static void setUp() throws IOException {
        db.setListingTablePath("src/test/java/database/Listings.csv");
        db.setUserTablePath("src/test/java/database/Users.csv");
        File usersCSV = new File(db.getUserTablePath());
        usersCSV.createNewFile();
        File listingsCSV = new File(db.getListingTablePath());
        listingsCSV.createNewFile();
    }

    @BeforeEach
    public void resetCSVFiles() throws IOException {
        File usersCSV = new File(db.getUserTablePath());
        if (usersCSV.delete()) {
            usersCSV.createNewFile();
        }
        File listingsCSV = new File(db.getListingTablePath());
        if (listingsCSV.delete()) {
            listingsCSV.createNewFile();
        }
        Listing.setNextId(0);
        User.setNextId(0);
    }

    @AfterAll
    public static void deleteCSVFiles() {
        File usersCSV = new File(db.getUserTablePath());
        usersCSV.delete();
        File listingsCSV = new File(db.getListingTablePath());
        listingsCSV.delete();
    }

    @Test
    public void currentUserHasListingInCartTest() throws IOException {
        db.createUser("currUser", "Pass", "email1");
        db.createListing("seller1", "title1", 1, LocalDate.EPOCH, "desc1", "imagePath1");
        db.createListing("seller2", "title2", 2, LocalDate.EPOCH, "desc2", "imagePath2");
        db.createListing("seller3", "title3", 3, LocalDate.EPOCH, "desc3", "imagePath3");
        User currUser = db.getUserWithUsername("currUser");
        Main.setCurrentUser(currUser);
        Listing listing1 = db.getListingByID(0);
        db.addListingToUserCart(Main.getCurrentUser(), listing1);
        User updatedUser = db.getUserWithUsername("currUser");

        assert (db.currentUserHasListingInCart(updatedUser, listing1));
    }

    @Test
    public void currentUserDoesNotHaveListingInCartTest() throws IOException {
        db.createUser("currUser", "Pass", "email1");
        db.createListing("seller1", "title1", 1, LocalDate.EPOCH, "desc1", "imagePath1");
        db.createListing("seller2", "title2", 2, LocalDate.EPOCH, "desc2", "imagePath2");
        db.createListing("seller3", "title3", 3, LocalDate.EPOCH, "desc3", "imagePath3");
        User currUser = db.getUserWithUsername("currUser");
        Listing listing1 = db.getListingByID(0);

        assert (!db.currentUserHasListingInCart(currUser, listing1));
    }

    @Test
    public void currentUserHasListingInCartOneInCartTest() throws IOException {
        db.createUser("currUser", "Pass", "email1");
        db.createListing("seller1", "title1", 1, LocalDate.EPOCH, "desc1", "imagePath1");
        db.createListing("seller2", "title2", 2, LocalDate.EPOCH, "desc2", "imagePath2");
        db.createListing("seller3", "title3", 3, LocalDate.EPOCH, "desc3", "imagePath3");
        User currUser = db.getUserWithUsername("currUser");
        Main.setCurrentUser(currUser);
        Listing listing1 = db.getListingByID(0);
        Listing listing2 = db.getListingByID(1);
        db.addListingToUserCart(Main.getCurrentUser(), listing1);
        User updatedUser = db.getUserWithUsername("currUser");

        assert (!db.currentUserHasListingInCart(updatedUser, listing2));
    }
}
